...
def error_handling(family: list, start: int, end: int) -> list:
	...
	if (isinstance(family, list) == False):
		...
		print(type(family))
		exit()
	first_column_length = len(family[0])
	equal_row_length = True
	for row in family:
		if (len(row) != first_row_length):
			equal_row_length = False
			break
	if equal_row_length == False:
		print("Error: List rows are not of equal length")
		exit()

# convert 2-D array list into a numpy 2-d array
# numpy 2-D array list must have the same row and column length
